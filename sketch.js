...
function preload() {
  ...
  backroundSound=loadSound("assets/background_music.mp3")
  explosionSound=loadSound("assets/cannon_explosion.mp3")
  splashSound=loadSound("assets/cannon_water.mp3")
  laughingSound=loadSound("assets/pirate_laugh.mp3")
}
...
function draw() {
  ...
  if(!backroundSound.isPlaying()){
    backroundSound.play()
    backroundSound.setVolume(0.02)
  }
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        if(!laughingSound.isPlaying()&&!isLaughing){
        laughingSound.play()
        isLaughing=true
        }
        isGameOver = true;
